...
export default function RegisterPage() {
  ...
  const onOAuthLogin = async (provider: "google" | "discord") => {
    ...
    try {
      ...
      if (!result.success) {
        setError(result.error || `Error al registrarse con ${provider}`);
        setIsLoading(false);
      }
            ...
            {registrationSuccess ? (
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="flex justify-center pt-2">
                  <Button
                    type="submit"
                    className="w-full md:w-auto md:min-w-[200px] bg-gold hover:bg-gold-dim text-background font-medium py-4 px-12 text-base cursor-pointer"
                    disabled={isLoading}
                  >
