  ...
  if (tipo === "lut" && Array.isArray(body?.lutCaras) && body.lutCaras.length > 0) {
    const lutRows = body.lutCaras
      ...
      .map((c: any) => ({
        ...
        tipo: c.tipo,
        cantidad_dados: c.tipo === "oro" ? (Number(c.cantidadDados) || 1) : null,
        tipo_dado_oro: c.tipo === "oro" ? (c.tipoDadoOro || "d6") : null,
        multiplicador_oro: c.tipo === "oro" ? (Number(c.multiplicadorOro) || 1) : 1,
