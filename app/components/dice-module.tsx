...
import { Coins, Package, List, Loader2, ChevronDown, ChevronUp } from "lucide-react";
import DiceVisual from "./dice-visual";
import FantasyAlert from "@/components/ui/fantasy-alert";
import type { DadoRecompensa, RollResult } from "@/lib/types/dados";
...
function rewardDescription(r: DadoRecompensa): string {
  if (r.tipo === "item_fijo") {
    return r.objetoNombre ?? "Ítem fijo";
  }
  if (r.tipo === "sublista") {
    return `${r.sublistaItems.length} ítems posibles`;
  }
  if (r.tipo === "oro_dados") {
    return `${r.cantidadDados}${r.tipoDado.toUpperCase()} × ${r.multiplicadorOro} oro`;
  }
  return "";
}
...
export default function DiceModule({ token, activeCharacterId }: Props) {
  ...
  const fetchConfig = useCallback(async () => {
    ...
    try {
      ...
      if (data.recompensas?.length > 0) {
        setSelectedId(data.recompensas[0].id);
      }
    } finally {
      setLoading(false);
    }
  ...
  async function handleRoll() {
    ...
    try {
      ...
      if (data.tipoResultado === "oro") {
        setAlert({
          variant: "success",
          message: `¡Obtuviste ${data.cantidadOro} de oro!`,
        });
      } else if (data.objeto) {
        setAlert({
          variant: "success",
          message: `¡Obtuviste ${data.objeto.nombre}!`,
        });
      }
    } catch {
      await new Promise((resolve) => setTimeout(resolve, 1400));
              ...
              {selectedReward && (
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    {Array.from({ length: selectedReward.tipo === "oro_dados" ? selectedReward.cantidadDados : 1 }).map(
                      (_, i) => (
                      ...
                      <div className="text-xs font-sans">
                        {rollResult.tipoResultado === "oro" ? (
                          ...
                          <span className="text-green-400 font-bold flex items-center gap-1">
                            {rollResult.objeto.icono && (
                              <span>{rollResult.objeto.icono}</span>
                            )}
